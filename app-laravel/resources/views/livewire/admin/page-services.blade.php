...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
        <span>{{ __('Tout ce que montre la page Services du site, rassemblé ici.') }}</span>
        <a href="{{ route('services.index') }}" target="_blank" rel="noopener"
    ...
    <div class="grid gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        <div class="space-y-6">
            <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                {{-- Deux modules n'ont pas d'en-tete de section : la page
                     n'affiche rien au-dessus de la grille des services, et
                     l'annonce est pilotee par son propre formulaire. Ni
                     bascule de langue ni formulaire, donc. --}}
                @if ($description['section'] ?? null)
                    ...
                    <form wire:submit="enregistrer" class="space-y-4">
                        ...
                        {{-- Les options d'apparence du bloc. Elles vivaient
                             dans le panneau de l'editeur des etapes, que
                             l'embarquement masque : sans cette reprise, la
                             mise en page serait devenue inatteignable. --}}
                        @foreach ($description['options'] ?? [] as $nom => $decrit)
                            <label class="block">
                                <span class="text-sm font-medium">{{ __($decrit['intitule']) }}</span>
                                <select wire:model="options.{{ $nom }}"
                                        class="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950">
                                    @foreach ($decrit['choix'] as $valeur => $libelle)
                                        <option value="{{ $valeur }}">{{ __($libelle) }}</option>
                                    @endforeach
                                </select>
                                @error('options.'.$nom) <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                            </label>
                        @endforeach
