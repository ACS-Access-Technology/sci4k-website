...
<div class="fab-stack" id="fabStack">
  <a class="fab-action fab-wa" href="https://wa.me/{{ $whatsappPublic }}?text={{ urlencode($whatsappMessage) }}" target="_blank" rel="noopener noreferrer" aria-label="{{ $t('aria_whatsapp', __('Discuter sur WhatsApp')) }}" title="WhatsApp">
  ...
  @if ($chatActif)
    <button class="fab-action fab-chat" type="button" id="fabChat" aria-label="{{ $t('aria_chat', __('Ouvrir le chat en ligne')) }}" title="{{ $t('titre_chat', __('Chat en ligne')) }}">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
    </button>
  @endif
  <button class="fab-trigger" type="button" id="fabTrigger" aria-expanded="false" aria-controls="fabStack" aria-label="{{ $t('aria_joindre', __('Nous joindre')) }}">
