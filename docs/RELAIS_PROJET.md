...
- La branche de travail est **`dev`**. Les trois branches permanentes sont `master`, `preprod` et `dev` ; l'intégration continue les surveille toutes les trois. Il n'existe pas de branche `prod` : `master` est la branche de production.
